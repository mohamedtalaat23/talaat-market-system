    ...
    <div className="flex-1 overflow-y-auto p-4" ref={listRef}>
      {cart.length === 0 ? (
        <div className="h-full flex flex-col items-center justify-center text-secondary">
          ...
          <p className="text-sm mt-2">or press <kbd className="px-2 py-1 bg-card-hover rounded text-foreground border border-border">F5</kbd> to search</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex-1">
                  <div className={`font-semibold ${isActive ? 'text-success' : 'text-foreground'}`}>{item.name}</div>
                  <div className="text-sm text-secondary">{item.barcode}</div>
                  {item.discount > 0 && (
                    <div className="text-xs text-danger mt-1">Discount: -EGP {item.discount.toFixed(2)}</div>
                  )}
                </div>
                ...
                <div className="flex items-center space-x-6">
                  ...
                  <div className="text-right min-w-[100px]">
                    <div className="text-sm text-secondary">{item.unit} x EGP {item.unit_price.toFixed(2)}</div>
                    <div className={`font-bold text-lg ${isActive ? 'text-success' : 'text-foreground'}`}>
