import { useEffect, useMemo, memo, useState } from 'react';
import { X, Clock, User as UserIcon, AlertTriangle, Save, Trash2 } from 'lucide-react';
import { useModalStore } from '@/stores/modalStore';
import { useFocusTrap } from '@/hooks/useFocusTrap';
import { usePOSStore } from '../usePOSStore';
import { useAuthStore } from '@/stores/authStore';
import toast from 'react-hot-toast';

interface SuspendedCartItemProps {
  cart: any;
  onDiscard: (holdId: string, cashierId: number) => void;
  onResume: (holdId: string, cashierId: number) => void;
}

const SuspendedCartItem = memo(({ cart, onDiscard, onResume }: SuspendedCartItemProps) => {
  const { total } = useMemo(() => {
    const sub = cart.cart.reduce(
      (sum: number, item: any) => sum + item.quantity * item.unit_price - item.discount,
      0,
    );
    return { total: sub - cart.globalDiscount };
  }, [cart]);

  return (
    <div className="bg-card-hover/50 rounded border border-border p-4 flex justify-between items-center">
      <div>
        <div className="flex items-center space-x-2 text-sm text-secondary mb-1">
          <Clock size={14} />
          <span>{new Date(cart.timestamp).toLocaleTimeString()}</span>
          <span className="px-2 text-muted">|</span>
          <UserIcon size={14} />
          <span>Cashier ID: {cart.cashier_id}</span>
        </div>
        <div className="font-bold text-foreground text-lg">
          {cart.cart.length} {cart.cart.length === 1 ? 'item' : 'items'}
          <span className="text-secondary font-normal ml-2">Total: EGP {total.toFixed(2)}</span>
        </div>
      </div>

      <div className="flex space-x-2">
        <button
          onClick={() => onDiscard(cart.hold_id, cart.cashier_id)}
          className="px-4 py-2 bg-danger/15 text-danger hover:bg-danger/25 rounded transition-colors"
        >
          Discard
        </button>
        <button
          onClick={() => onResume(cart.hold_id, cart.cashier_id)}
          className="px-6 py-2 bg-warning/90 hover:bg-warning text-white font-bold rounded transition-colors"
        >
          Resume
        </button>
      </div>
    </div>
  );
});

export function SuspendedCartsModal() {
  const isOpen = useModalStore((state) => state.activeModals.pos_suspended_carts);
  const closeModalAction = useModalStore((state) => state.closeModal);
  const openModalAction = useModalStore((state) => state.openModal);
  const closeModal = () => {
    setOverwriteTarget(null);
    closeModalAction('pos_suspended_carts');
  };

  const heldCarts = usePOSStore((state) => state.heldCarts);
  const resumeCart = usePOSStore((state) => state.resumeCart);
  const removeHeldCart = usePOSStore((state) => state.removeHeldCart);
  const holdCurrentCart = usePOSStore((state) => state.holdCurrentCart);
  const clearCart = usePOSStore((state) => state.clearCart);
  const currentCartLength = usePOSStore((state) => state.cart.length);

  const user = useAuthStore((state) => state.user);
  const focusTrapRef = useFocusTrap<HTMLDivElement>(isOpen);

  const [overwriteTarget, setOverwriteTarget] = useState<{holdId: string, cashierId: number} | null>(null);

  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        if (overwriteTarget) setOverwriteTarget(null);
        else closeModal();
      }
      
      // Keyboard-first workflow for overwrite protection
      if (overwriteTarget && isOpen) {
        if (e.key.toLowerCase() === 's') {
          e.preventDefault();
          handleSuspendCurrentAndResume();
        } else if (e.key.toLowerCase() === 'd') {
          e.preventDefault();
          handleDiscardCurrentAndResume();
        } else if (e.key.toLowerCase() === 'c') {
          e.preventDefault();
          setOverwriteTarget(null);
        }
      }
    };
    if (isOpen) window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [isOpen, overwriteTarget]);

  if (!isOpen) return null;

  const handleResumeClick = (holdId: string, cashierId: number) => {
    if (user?.id !== cashierId && user?.role !== 'admin' && user?.role !== 'manager') {
      closeModal();
      openModalAction('pos_manager_override', {
        action: 'cross_cashier_resume',
        holdId,
      });
      return;
    }

    if (currentCartLength > 0) {
      setOverwriteTarget({ holdId, cashierId });
    } else {
      executeResume(holdId);
    }
  };

  const handleDiscardClick = (holdId: string, cashierId: number) => {
    if (user?.id !== cashierId && user?.role !== 'admin' && user?.role !== 'manager') {
      toast.error("Only managers can discard other cashiers' suspended carts");
      return;
    }
    if (window.confirm('Are you sure you want to permanently delete this suspended cart?')) {
      removeHeldCart(holdId);
      toast.success('Suspended cart discarded');
    }
  };

  const executeResume = (holdId: string) => {
    resumeCart(holdId);
    toast.success('Suspended cart resumed');
    closeModal();
  };

  const handleSuspendCurrentAndResume = () => {
    if (!overwriteTarget) return;
    holdCurrentCart(user?.id || 1);
    executeResume(overwriteTarget.holdId);
  };

  const handleDiscardCurrentAndResume = () => {
    if (!overwriteTarget) return;
    clearCart();
    executeResume(overwriteTarget.holdId);
  };

  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="absolute inset-0" onClick={() => overwriteTarget ? setOverwriteTarget(null) : closeModal()} aria-hidden="true" />

      <div
        ref={focusTrapRef}
        className="w-full max-w-2xl rounded-lg border border-border bg-card p-6 shadow-2xl relative z-10 animate-fade-in flex flex-col max-h-[80vh]"
        role="dialog"
      >
        {!overwriteTarget ? (
          <>
            <div className="flex items-center justify-between border-b border-border pb-3 mb-4">
              <h3 className="text-xl font-bold text-white flex items-center space-x-2">
                <Clock size={24} className="text-warning" />
                <span>Suspended Carts</span>
              </h3>
              <button
                onClick={closeModal}
                className="text-secondary hover:text-white transition-colors"
              >
                <X size={20} />
              </button>
            </div>

            <div className="flex-1 overflow-y-auto space-y-3">
              {heldCarts.length === 0 ? (
                <div className="py-12 text-center text-secondary">
                  <p>No suspended carts found.</p>
                </div>
              ) : (
                heldCarts.map((cart) => (
                  <SuspendedCartItem
                    key={cart.hold_id}
                    cart={cart}
                    onDiscard={handleDiscardClick}
                    onResume={handleResumeClick}
                  />
                ))
              )}
            </div>
          </>
        ) : (
          <div className="py-6 px-4 flex flex-col items-center text-center animate-scale-in">
            <div className="w-16 h-16 bg-warning/10 rounded-full flex items-center justify-center mb-4">
              <AlertTriangle className="text-warning w-8 h-8" />
            </div>
            <h3 className="text-2xl font-bold text-white mb-2">Active Cart Overwrite</h3>
            <p className="text-secondary text-lg mb-8">
              Current cart contains <span className="text-white font-bold">{currentCartLength}</span> items.<br/>
              Resuming will overwrite the active cart.
            </p>

            <div className="flex flex-col w-full max-w-sm space-y-3">
              <button 
                onClick={handleSuspendCurrentAndResume}
                className="flex items-center justify-center space-x-2 w-full py-3 bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-lg transition-colors focus:ring-2 focus:ring-offset-2 focus:ring-offset-background focus:ring-primary"
              >
                <Save className="w-5 h-5" />
                <span>[S] Suspend Current Cart</span>
              </button>
              
              <button 
                onClick={handleDiscardCurrentAndResume}
                className="flex items-center justify-center space-x-2 w-full py-3 bg-danger/20 hover:bg-danger/30 text-danger font-bold border border-danger/30 rounded-lg transition-colors focus:ring-2 focus:ring-offset-2 focus:ring-offset-background focus:ring-danger"
              >
                <Trash2 className="w-5 h-5" />
                <span>[D] Discard Current Cart</span>
              </button>
              
              <button 
                onClick={() => setOverwriteTarget(null)}
                className="w-full py-3 text-secondary hover:text-white font-bold transition-colors mt-2"
              >
                [C] Cancel
              </button>
            </div>
          </div>
        )}
      </div>
    </div>
  );
}
