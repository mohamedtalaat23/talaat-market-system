...
import { useInventory, useInventoryCategories, type InventoryItem } from './hooks/useInventoryQueries';
...
export function InventoryPage() {
  ...
  const { data: inventoryData, isLoading: isLoadingInventory, error, refetch } = useInventory(queryFilters);
  ...
  return (
    <PageContainer
      ...
      description="Monitor physical store stock counts, configure warning limits, and audit logs of adjustments."
      ...
      actions={
        <Button
          ...
          variant="outline"
          className="flex items-center space-x-1.5 font-semibold h-10"
        >
          <History size={16} />
          <span>Global History Log</span>
        </Button>
      }
    >
      ...
      <InventoryFilterBar
        ...
        onSelectCategory={(catId) => { setSelectedCategoryId(catId); setPage(1); }}
        debouncedSearch={debouncedSearch}
        onSearchChange={(search) => { setDebouncedSearch(search); setPage(1); }}
        showLowStock={showLowStock}
        onToggleLowStock={() => { setShowLowStock(!showLowStock); setPage(1); }}
