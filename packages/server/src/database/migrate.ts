...
async function run() {
  ...
  try {
    ...
    logger.error(
      'Please check that PostgreSQL is installed, running, and credentials in .env are correct.',
    );
    process.exit(1);
  }
  ...
  try {
    if (command === 'rollback') {
      ...
      logger.info(
        'ℹ️ Note: Rollback runs inside a transaction. If it fails, all changes are fully rolled back.',
      );
      ...
      logger.info(
        `Status: ${completed.length} applied, ${pending.length} pending migrations found.`,
      );
      ...
      if (pending.length > 0) {
        ...
        logger.info(
          'ℹ️ Note: PostgreSQL uses transactional DDL. If any migration fails, the entire batch',
        );
